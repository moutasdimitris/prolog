...
query(ListOfKeywords):-
    prepare(X),prepare1(Y),
    check(X,Y,ListOfKeywords,Score_Title,Score_Topics),
    printing(Score_Title,Score_Topics).
...
printing(Score_Title,Score_Topics):-
    ...
    write("Relevance = "),
    Score is (Score_Title + Score_Topics),
    write(Score).
...
score_by_one_word_title(H2,H,Score):-
weight(H2,Weight),(sub_string(case_insensitive,'-',H2)->remove_weight(H2,L),(sub_string(case_insensitive,L,H)->Score is 2*Weight);(sub_string(case_insensitive,H2,H)->Score is 2*Weight)).


score_by_one_word_topics(H2,[H|T],Score):-
weight(H2,Weight),(sub_string(case_insensitive,'-',H2)->remove_weight(H2,L),(sub_string(case_insensitive,L,H)->Score is Weight);score_by_one_word_topics(H2,T,Score)).
...
check([],[],[],_,_).
check([H|T],[H1,T1],[H2,T2],Score_List,Sc):-
    score_by_title(H,H2,Score),
    score_by_topics(H1,H2,[],Title_score),
    check(T,T1,T2,Score_List, Sc),
    Score_List is Title_score,write(Score_List),nl,write(Score),
    Sc is Score.
